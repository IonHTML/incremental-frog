...
function buyGlobalmult() {
    if (frogs >= globalMult.cost)
    {
        frogs -= globalMult.cost;
        globalMult.mult += .1;

        document.getElementById("frogs").innerHTML = "You have " + format(frogs) + " frogs.";
        document.getElementById("globalMulti").innerHTML = "Increase Multi | Cost: " + format(globalMult.cost) + " | Mult: " + globalMult.mult + "x"
    }
}
...
function buyPrestige() {
    if (frogs >= prestige1.cost) {
        ...
        gen4.cost = 1000000;
        gen5.cost = 100000000;
        gen6.cost = 1000000000;
        globalMult.mult = 1;
        globalMult.cost = 1000;
        gen1.mult += 1;
        gen2.mult += 1;
        gen3.mult += 1;
        gen4.mult += 1;
...
function load() {
    ...
    if (typeof savegame.sglobalMult !== "undefined") globalMult = savegame.sglobalMult;
}
...
setInterval(function () {
    frogs = Math.round(frogs + gen1.amount * gen1.mult * globalMult.mult);
    fps = Math.round(gen1.amount * gen1.mult * globalMult.mult * globalMult.mult)                                  
    gen5.amount = gen5.amount + gen6.amount * gen6.mult * globalMult.mult;
    gen4.amount = gen4.amount + gen5.amount * gen5.mult * globalMult.mult; 
    gen3.amount = gen3.amount + gen4.amount * gen4.mult * globalMult.mult;
    gen2.amount = gen2.amount + gen3.amount * gen3.mult * globalMult.mult;
    gen1.amount = gen1.amount + gen2.amount * gen2.mult * globalMult.mult;
    document.getElementById("frogs").innerHTML = "You have " + format(frogs) + " frogs.";
    document.getElementById("fps-counter").innerHTML = "You are getting " + format(fps) + " frogs per " + second + " ms"
    document.getElementById("prestige").innerHTML = "Prestige | Cost: " + format(prestige1.cost) + "| Mult: " + format(prestige1.mult) + "x";
    document.getElementById("generator1").innerHTML = "Buy Gen 1 | Cost: " + format(gen1.cost) + " | Amount: " + format(gen1.amount) + " | Mult: " + format(globalMult.mult) + "x";
    document.getElementById("generator2").innerHTML = "Buy Gen 2 | Cost: " + format(gen2.cost) + " | Amount: " + format(gen2.amount) + " | Mult: " + format(globalMult.mult) + "x";
    document.getElementById("generator3").innerHTML = "Buy Gen 3 | Cost: " + format(gen3.cost) + " | Amount: " + format(gen3.amount) + " | Mult: " + format(globalMult.mult) + "x";
    document.getElementById("generator4").innerHTML = "Buy Gen 4 | Cost: " + format(gen4.cost) + " | Amount: " + format(gen4.amount) + " | Mult: " + format(globalMult.mult) + "x";
    document.getElementById("generator5").innerHTML = "Buy Gen 5 | Cost: " + format(gen5.cost) + " | Amount: " + format(gen5.amount) + " | Mult: " + format(globalMult.mult) + "x";
    document.getElementById("generator6").innerHTML = "Buy Gen 6 | Cost: " + format(gen6.cost) + " | Amount: " + format(gen6.amount) + " | Mult: " + format(globalMult.mult) + "x";
    document.getElementById("globalMulti").innerHTML = "Increase Multi | Cost: " + format(globalMult.cost) + " | Mult: " + globalMult.mult + "x"
}, second);
